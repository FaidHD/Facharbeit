    ...
    def wait_for_command_input(self):  # Verwaltung der Nutzereingaben außerhalb des Menüs
        ...
        for command in self.registered_commands: # Durchgehen durch alle registrierten Befehle
            if command.name == name:
                ...
                return
        cO.Output(["Command nicht gefunden. Um eine Liste an Befehlen zu erhalten, nutze \"help\""]).printString() # Falls kein Befehl gefunden wurde
        self.wait_for_command_input() # Erneute Ausführung dieser Methode
...
class Command:
    ...
    def call(self, args):
        cO.Output(["Call method from command" + self.name + " not defined"]).printString() # Falls die call Methode nicht überschrieben wird


class HelpCommand(Command):

    def __init__(self, main_instance):
        super().__init__("help", "Liste alle verfügbaren Befehle auf") # Initialisierung des übergeordneten Command Objekts
    ...
    def call(self, args):
        ...
        for command in self.main_instance.command_manager.get_commands(): # Durch alle registrierten Befehle iterieren
            strings.append(f"{command.name} - {command.description}") # Befehl als String in die Nutzerausgabe hinzufügen
    ...
    def call(self, args):
        if len(args) == 4: # Überprüfung der Anzahl der Argumente, da exakt 4 für diesen Command benötigt werden
            try: # Da hier Strings zu Integern gemacht werden müssen
                ...
                sad.CreateSeed(name, wachszeit, kornabstand, reihenabstand) # Übergabe der Argumente zur finalen Erstellung
                cO.Output([f"Die Saat {name} wurde erfolgreich erstellt"]).printString() # Ausgabe für den Nutzer
            except ValueError:
                cO.Output(["Bitte benutze: createSeed <Name> <Wachszeit> <Kornabstand> <Reihenabstand>"]).printString() # Falls die Argumente nicht richtig verarbeitet werden konnten
        else:
            cO.Output(["Bitte benutze: createSeed <Name> <Wachszeit> <Kornabstand> <Reihenabstand>"]).printString() # Falls die Anzahl der Argumente nicht 4 entspricht
...
class CreateFieldCommand(Command):
    ...
    def call(self, args):
        if len(args) != 2: # Überprüfung der Anzahl der Argumente, da exakt 2 für diesen Command benötigt werden
            cO.Output(["Bitte benutze createField <height> <width>"]).printString() # Falls die Anzahl der Argumente nicht 2 entspricht
            return
        else:
...
class ShowSeedsCommand(Command):
    ...
    def call(self, args):
        ...
        for i in result: # Durch die Ergebnisse der Datenbank iterieren
            strings.append(f"Name: {i[0]} | Wachszeit: {i[1]} | Kornabstand: {i[2]} | Reihenabstand: {i[3]}") # Hinzufügen der einzelnen Teile in die Ausgabe-Liste
...
class ShowFieldsCommand(Command):
    ...
    def call(self, args):
        ...
        strings = [] # Erstellung der Liste für die Ausgabe
        for i in result: # Durch die Ergebnisse der Datenbank iterieren
            strings.append(f"Id: {i[0]} | Breite: {i[2]} | Höhe: {i[1]}") # Hinzufügen der einzelnen Teile in die Ausgabe-Liste
        cO.Output(strings).printString() # Ausgabe der Liste
